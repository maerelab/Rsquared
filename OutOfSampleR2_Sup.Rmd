---
title: "Supplementary material for 'Out-of-sample \\rsq: estimation and inference'"
...
libs = c("ggplot2", "parallel", "reshape2", "nlme", "glmnet", "xtable")
for (i in libs){
...
methodLevels <- c("bootRhos", "bootRhosParam", "cvRhos", "jnRhos", "oracleRho", "bootSEs", "bootParamSEs", "oracleSE", "bootPercentile", "bootPercentileParam", "bootparamPercentile", "boot", "bootparam", "BCa", "BCaParam")
methodLabels <- c("Delta method SE (nonparametric bootstrap)", "Delta method SE (parametric bootstrap)", "Delta method SE (cross-validation)", "Delta method SE (jackknife)", "Delta method SE (oracle)", "Bootstrap SE (nonparametric)", "Bootstrap SE (parametric)", "Oracle SE", "Percentile bootstrap", "Percentile parametric bootstrap", "Percentile parametric bootstrap", "Percentile bootstrap", "Percentile parametric bootstrap", "BCa", "BCa (parametric)")
...
biasSEplot = ggplot(data = biasProcSE[with(biasProcSE, cvSplitsSweep %in% numSplitsShow & bootReps >= 50 & betas %in% c(0, 1, 1.5)), ], aes_string(y = "Bias", x = "samSize", colour = "Method", linetype = "factor(cvSplitsSweep)")) + geom_line() + facet_grid(betas ~ bootReps, scale = "free_y") + guides(linetype = guide_legend("Number of cross-validation splits")) + 
    xlab("Sample size") + ylab(bquote('Log10 of the geometric mean of the ratio of estimated to approximated true SE of '*R^2)) + geom_hline(yintercept = 0, linetype = "dotted") + scale_linetype_manual(values = linetypesShow) + scale_colour_manual(values = methodColours[names(methodColours) %in% SEplot]) + scale_x_continuous(breaks = samSize) + theme(axis.text.x = element_text(angle=60))
ggsave(biasSEplot, file = "Graphs/biasSE.pdf", height = 6.5)
```

```{r risweepCov, fig.cap = 'Coverage of the confidence intervals (y-axis) for cross-validation for different sample sizes (x-axis), methods (colour), signal strength (side panel) and number of bootstrap replicates (top panels) for 200 repeats of cross-validation splits. The dotted line indicates the nominal coverage of 95\\%.\\label{fig:sweepCov}', fig.height = 6, fig.width = 8}
...
ggplot(data = trueSEsAll[trueSEsAll$bootReps==500, ], mapping = aes_string(x = "samSize", y = "SER2", linetype = "factor(cvSplitsSweep)")) + geom_line() + scale_linetype_manual(values = linetypes) + guides(linetype = guide_legend("Number of cross-validation splits")) + 
    facet_grid(~betas) + xlab("Sample size") + ylab(bquote("Standard error of "*R^2*" estimator"))
```
...
names(dimnames(R2cast)) <- c("Effect size", "Sample size")
print.xtableFtable(xtableFtable(ftable(round(R2cast,3), method = "non.compact"), digits = 3, caption = "Monte-Carlo approximated true standard erros (SE) of $R^2$ estimated through the .632 bootstrap for the one-dimensional scenario for different sample sizes (columns) and effect sizes $\\beta$ (rows) for 200 inner bootstrap instances.\\label{tab:trueSER2boot}"), comment = FALSE)
```

```{r trueSEfigBoot, fig.cap = "Monte-Carlo approximated true standard errors (SE) of \\hrsq estimated through the .632 bootstrap (y-axis) for the one-dimensional scenario for different sample sizes (x-axis), effect sizes $\\beta$ (top panels) and number of .632 bootstraps (line type). \\label{fig:trueSER2boot}", fig.height = 3.5}
...
```{r compareSEs, fig.cap = "\\label{fig:seMSE}Approximated true standard errors of \\hrsq (y-axis) in the one-dimensional scenario for cross-validation and .632 bootstrap estimates (colour) for different sample sizes (x-axis) and signal strengths (top panels) over 1,000 Monte-Carlo instances. The number of cross-validation splits was 25, and the number of .632 bootstraps was 200; with these settings, the cross-validation takes about twice as long as the .632 bootstrap to estimate \\rsq and its SE.", fig.height = 4}
...
ggplot(data = trueMSEmerge, aes(x = samSize, y = trueSE, colour = Data_splitting_algorithm)) + geom_point() + facet_grid(~betas) +  ylab(bquote("True standard error of "*R^2)) + xlab("Sample size") + scale_x_continuous(breaks = unique(trueSEboot$samSize)) + theme(axis.text.x = element_text(angle = 60))
```
...
if(!file.exists(avR2File <- "Results/avR2.RData")){
    avR2s = mclapply(mc.cores = nCores, seq_len(nrow(gridAv)), function(j){
        vapply(integer(nSimAv), FUN.VALUE = double(3), function(i){
            ...
            c("Pooling R²" = 1-mean(R2s["MSE",])/MSTtot, "Averaging R²\ntraining MST" = mean(R2s[ "R2",]), 
...
boxColours = c("Pooling R²" = "grey45", "Averaging R²\ntraining MST" = "grey72", "Averaging R²\ntest MST" = "black")
ggplot(data = moltAvR2, aes(y = value, col = variable, x = variable)) + geom_boxplot() +
    ...
    scale_colour_manual(name = "Estimation method", values = boxColours) +
    theme(axis.text.x = element_blank(), axis.ticks.x = element_blank(),legend.text = element_text(
...
ggplot(data = biasAllAgg, aes_string(y = "biasR2", x = "samSize")) + geom_line() + facet_grid(betas ~ Resampling_method, scale = "free_y") + guides(linetype = guide_legend("Number of cross-validation splits")) + 
    xlab("Sample size") + ylab(bquote('Bias of '*R^2*' estimation')) + scale_linetype_manual(values = linetypes) +  scale_x_continuous(breaks = samSize) + theme(axis.text.x = element_text(angle=60)) + geom_hline(yintercept = 0, linetype = "dotted")
```
...
ggplot(data = biasProcSE, aes_string(y = "MSESE", x = "samSize", colour = "Method", linetype = "factor(cvSplitsSweep)")) + geom_line() + facet_grid(betas ~ bootReps, scale = "free_y")+ guides(linetype = guide_legend("Number of cross-validation splits")) + 
    xlab("Sample size") + ylab(bquote('MSE of the estimated SE of '*R^2)) + scale_linetype_manual(values = linetypes) + scale_colour_manual(values = methodColours[names(methodColours) %in% SEplot]) + scale_y_log10()  + scale_x_continuous(breaks = samSize) + theme(axis.text.x = element_text(angle=60))
...
ggplot(data = biasProcSEboot, aes_string(y = "BiasSE", x = "samSize", colour = "Method", linetype = "factor(bootRepsOuter)")) + geom_line() + facet_grid(betas ~ bootReps, scale = "free_y")+ guides(linetype = guide_legend("Number of outer bootstrap repeats")) + 
    xlab("Sample size") + ylab(bquote('Log10 of the geometric mean of the ratio of estimated to the approximated true SE of '*R^2)) + geom_hline(yintercept = 0, linetype ="dotted") + scale_linetype_manual(values = linetypesBoot) + 
...
```{r trueSEfigCvHD, fig.cap = "Monte-Carlo approximated true standard errors (SE) of \\hrsq (y-axis) estimated through cross-validation in the high-dimensional scenario for different sample sizes (x-axis), effect sizes $\\beta$ (top panels) and number of cross-validation splits (line type). The SE increases with the sample size because also true \\rsq increases with sample size (Table \\ref{tab:trueR2HD}).\\label{fig:trueSER2HD}", fig.height=3.5}
...
### Alternative estimators for \rsq \label{sec:altEst}

```{r r2estHigh, fig.cap = "Boxplots of \\rsq values estimated in three different ways (greyscale) for the high-dimensional scenario using cross-validation (x-axis, see Section \\ref{sec:estR2} in the main text) for different sample sizes (side panels) and effect sizes (top panels). Simple, non-nested CV is used to estimate the MSE. Diamonds indicate means; the plot was trunctated at -1 for legibility. The dashed horizontal line indicates true \\rsq approximated through Monte-Carlo simulation from Table \\ref{tab:trueR2HD}.\\label{fig:altR2high}", fig.height = 7}
...
if(!file.exists(avR2highFile <- "Results/avR2high.RData")){
    avR2highs = mclapply(mc.cores = nCores, seq_len(nrow(gridAvHigh)), function(j){
        vapply(integer(nSimAvHigh), FUN.VALUE = double(3), function(i){
            ...
            MSTtot = var(trainDat$y)*(nAv+1)/nAv
            c("Pooling R²" = 1-mean(R2s["MSE",])/MSTtot, "Averaging R²\ntraining MST" = mean(R2s[ "R2",]), "Averaging R²\ntest MST" = mean(R2s[ "R2test",]))
        })
    })
...
```{r R2EstimationHD, fig.height = 4, fig.cap = "Bias of \\rsq estimation (y-axis) through cross-validation and bootstrap (top panels) using the pooling estimator over 100 Monte-Carlo instances for the high-dimensional scenario as a function of sample size (x-axis) and signal strength (side panels) for 100 cross-validation splits and 200 bootstraps. The horizontal dotted line at 0 indicates no bias. At high sample sizes and effect sizes, out-of-sample \\rsq is underestimated, implying a conservative assessment of predictive performance. This bias may contribute to the poor coverage of the confidence intervals for high sample sizes (\\fref{fig:HighDim}).\\label{fig:biasR2HD}", fig.width = 4.2}
...
```{r checkSEhigh, fig.height = 6, fig.cap="Geometric mean of log10 ratio of the estimated to the Monte-Carlo approximated true SE of cross-validation \\rsq over 100 Monte-Carlo instances for the high-dimensional scenario (y-axis) as a function of sample size (x-axis), number of bootstrap replicates (top panels) and signal strength (side panels). The dotted line at 0 is a visual aid.\\label{fig:biasHigh}"}
...
ggplot(data = biasProcSENMolt[biasProcSENMolt$Method %in% labels2plotHigh,], aes_string(y = "BiasSE", x = "samSize", colour = "Method", linetype = "factor(cvSplits)")) + geom_line() + facet_grid(betas ~ bootReps, scale = "free_y")+ guides(linetype = guide_legend("Number of cross-validation splits"))+
    xlab("Sample size") + ylab(bquote("Log10 ratio of the estimated SE of "*R^2*" to the approximated true SE")) + geom_hline(yintercept = 0, size = 0.5, linetype = "dotted") + scale_linetype_manual(values = linetypesHigh)+
...
```{r checkSEhighMSE, fig.cap="MSE of the estimated SE of cross-validation \\rsq for the high-dimensional scenario (y-axis) as a function of sample size (x-axis), number of bootstrap replicates (top panels) and signal strength (side panels). The y-axis is on the log scale.\\label{fig:mseHigh}"}
...
ggplot(data = biasProcSENMoltmse[biasProcSENMoltmse$Method %in% labels2plotHigh,], aes_string(y = "BiasSE", x = "samSize", colour = "Method", linetype = "factor(cvSplits)")) + geom_line() + facet_grid(betas ~ bootReps, scale = "free_y") + guides(linetype = guide_legend("Number of cross-validation splits")) +
    xlab("Sample size") + ylab(bquote("MSE of estimated SE of "*R^2)) + scale_linetype_manual(values = linetypesHigh) + scale_y_log10() +
    scale_colour_manual(values = methodColours[names(methodColours) %in% labels2plotHigh[1:5]]) + scale_x_continuous(breaks = samSizeHigh) + theme(axis.text.x = element_text(angle=60))
```
...
ggplot(data = sweepSumMoltHigh[with(sweepSumMoltHigh, cvSplits==100 & Method %in% labels2plotHigh),], aes(x = samSize, y = Coverage, col = Method, )) + geom_line() + facet_grid(betas~ bootReps, scale = "free_y") +
    geom_hline(yintercept = 1-sigLevel, linetype = 'dotted')+ ylab(bquote("Coverage of the 95% confidence intervals for "*R^2)) + scale_colour_manual(values = methodColours[names(methodColours) %in% labels2plotHigh]) + xlab("Sample size") + scale_x_continuous(breaks = samSizeHigh) + theme(axis.text.x = element_text(angle=60))
```

```{r plotwidthsweepHD, fig.cap = 'Average width of the 95\\% confidence intervals for cross-validation \\rsq (y-axis) for different sample sizes (x-axis), method (colour), signal strength (side panel) and number of bootstrap replicates (top panels) for 100 repeats of cross-validation splits for the high-dimensional scenario.\\label{fig:sweepWidthHD}'}
...
ggplot(data = sweepSumMoltW[with(sweepSumMoltW,cvSplits==100 & Method %in% labels2plotHigh),], aes(x = samSize, y = Width_of_confidence_interval, col = Method)) + geom_line() + facet_grid(betas~ bootReps, scale = "free_y")+ ylab(bquote("Width of the confidence interval for "*R^2)) + xlab("Sample size") +
    ylab(bquote("Average width of the 95% confidence intervals for "*R^2)) +
...
```{r checkSEhighBoot, fig.cap= "Log10 ratio of the estimated to the Monte-Carlo approximated true SE of .632 bootstrap \\rsq  (y-axis) across 100 Monte-Carlo instances for the high-dimensional scenario as a function of sample size (x-axis), number of bootstrap replicates (top panels) and signal strength (side panels). The dotted line at 0 is a visual aid. The delta method SE with nonparametric bootstrap or jackknife is the only method that never underestimates the SE.\\label{fig:biasHighBoot}"}
...
ggplot(data = biasProcSENMolt[biasProcSENMolt$Method %in% labels2plotHigh,], aes_string(y = "BiasSE", x = "samSize", colour = "Method", linetype = "factor(bootRepsOuter)")) + geom_line() + facet_grid(betas ~ bootReps, scale = "free_y")+ guides(linetype = guide_legend("Number of outer bootstraps"))+
    xlab("Sample size") + ylab(bquote("Log10 ratio of the estimated SE of "*R^2*" to the approximated true SE")) + geom_hline(yintercept = 0, size = 0.5, linetype = "dotted") +
    scale_colour_manual(values = methodColours[names(methodColours) %in% labels2plotHigh[1:5]]) + scale_x_continuous(breaks = samSizeHigh) + theme(axis.text.x = element_text(angle=60))
```

```{r checkSEhighMSEboot, fig.cap="MSE of the estimated SE of .632 bootstrap \\rsq for the high-dimensional scenario (y-axis) as a function of sample size (x-axis), number of bootstrap replicates (top panels) and signal strength (side panels). The y-axis is on the log scale.\\label{fig:mseHighBoot}"}
...
biasProcSENMolt$Method = factor(biasProcSENMolt$Method, levels = methodLevels, 
                             labels = methodLabels)
ggplot(data = biasProcSENMolt[biasProcSENMolt$Method %in% labels2plotHigh,], aes_string(y = "BiasSE", x = "samSize", colour = "Method", linetype = "factor(bootRepsOuter)")) + geom_line() + facet_grid(betas ~ bootReps, scale = "free_y")+ guides(linetype = guide_legend("Number of outer bootstraps"))+
    xlab("Sample size") + ylab(bquote("MSE of estimated SE of "*R^2)) + scale_y_log10() +
    scale_colour_manual(values = methodColours[names(methodColours) %in% labels2plotHigh[1:5]]) + scale_x_continuous(breaks = samSizeHigh) + theme(axis.text.x = element_text(angle=60))
```
