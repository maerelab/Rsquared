...
nDigits = 2
R2sSEsciCop[, !grepl(colnames(R2sSEsciCop), pattern =  "P-value")] = apply(R2sSEsciCop[, !grepl(colnames(R2sSEsciCop), pattern =  "P-value")], 1:2, round, digits = nDigits)
print(xtable(R2sSEsciCop, digits = nDigits, align = "l|ccccccccc", display = c("s", "f", "f", "e", rep("f", 3), "e", "f", "f"), caption = "\\label{tab:R2se}Estimated \\rsq, delta method and nonparametric bootstrap standard error (SE), one-sided p-value and lower and upper bound of the 95\\% confidence interval for 5 \\textit{Brassica napus} phenotypes."), comment = FALSE)
#Zea mays
R2sSEsciZMCop = R2sSEsciZM
rownames(R2sSEsciZMCop) = gsub("_", " ", rownames(R2sSEsciZMCop))
colnames(R2sSEsciZMCop)[1] = "R²"
print(xtable(R2sSEsciZMCop, digits = nDigits, align = "l|ccccccccc", display = c("s", "f", "f", "e", rep("f", 3), "e", "f", "f"), caption = "\\label{tab:R2seZM}Estimated \\rsq, delta method and nonparametric bootstrap standard error (SE), one-sided p-value between brackets and lower and upper bound of the 95\\% confidence interval for 5 \\textit{Zea mays} phenotypes."), comment = FALSE)
colnames(R2sSEsciZMCop)[1] = "R²"
```
...
```{r compareSEs, fig.cap = "\\label{fig:seMSE}Approximated true standard errors of the \\hrsq (y-axis) for cross-validation and .632 bootstrap estimates (colour) for different sample sizes (x-axis) and signal strengths (top panels) over 1,000 Monte-Carlo instances. The number of cross-validation splits was 25, and the number of .632 bootstraps was 200; with these settings, the cross-validation takes about twice as long as the .632 bootstrap to estimate the \\rsq and its SE.", fig.height = 4}
...
```{r r2average, fig.cap = "Boxplots of \\rsq values estimated in three different ways using cross-validation (x-axis, see Section \\ref{sec:estR2} in the main text) for different sample sizes and effect sizes (see plot titles). Simple, non-nested CV is used to estimate the MSE, the rest of the setup is the same as for the one-dimensional scenario simulations with exclusion of the sample size 30. Diamonds indicate means; the plot was trunctated at -2 for legibility. The dashed horizontal line indicates the true \\rsq approximated through Monte-Carlo simulation from Table \\ref{tab:trueR2}.\\label{fig:altR2}", fig.height = 9}
nSimAv = 1e3 
...
par(mfrow = c(4,3), mar = c(3, 1.8, 2.5, 1))
for(j in seq_len(nrow(gridAv))){
    boxplot(t(avR2s[[j]]), main = paste0("Sample size ", gridAv[j, "samSize"], "\neffect size ", gridAv[j, "betas"]),
            ylim = c(max(min(avR2s[[j]]), -2), max(avR2s[[j]])), cex.axis = 0.9)
    points(seq_len(nrow(avR2s[[j]])), pch = 5, rowMeans(avR2s[[j]]))
    abline(h = R2castTrue[as.character(gridAv[j, "betas"]), as.character(gridAv[j, "samSize"])], lty = "dashed")
}
par(mfrow = c(1,1))
```
...
```{r biasR2, fig.cap = "Estimation error of the \\rsq estimation (y-axis) through cross-validation and bootstrap (top panels) over 1,000 Monte-Carlo instances as a function of sample size (x-axis) and signal strength (side panels) for 200 cross-validation splits and 200 .632 bootstrap instances. Bias decreases with growing sample size and for .632 bootstrap with growing effect size. \\label{fig:biasR2Uni}", fig.width = 4.2}
...
```{r varMSE, fig.cap = "Mean difference between the cross-validation Var(MSE) estimator by \\textcite{Bates2021} and approximated true variance  of the MSE over 1,000 Monte-Carlo instances (y-axis) as a function of  sample size (x-axis), number of cross-validation splits (top panels) and signal strength (side panels). The dotted line at zero indicates no bias. For sufficiently large sample sizes and number of cross-validation splits, the estimator of the variance is nearly unbiased.\\label{fig:biasMSESE}"}
...
```{r varMSEBoot, fig.cap = "Mean difference between the .632 bootstrap Var(MSE) estimator by \\textcite{Efron1997} and approximated true variance of the MSE over 1,000 Monte-Carlo instances (y-axis) as a function of  sample size (x-axis), number of bootstraps (top panels) and signal strength (side panels). The dotted line at zero indicates no bias. The bias decreases with number of bootstrap samples and sample size.\\label{fig:biasMSESEboot}"}
...
```{r plotwidthsweep, fig.cap = 'Average width of the 95\\% confidence intervals for \\rsq across 1,000 Monte-Carlo instances (y-axis) for cross-validation for different sample sizes (x-axis), method (colour), signal strength (side panel) and number of bootstrap replicates (top panels) for 200 repeats of the cross-validation splits. The number of bootstraps has little effect on the confidence intervals of the methods based on the delta method SE.\\label{fig:sweepWidth}', fig.height = 6, fig.width = 7}
...
#### The .632 bootstrap
<!-- Bootstrap .632 results are shown in Figures \ref{fig:biasUniBoot}-\ref{fig:zTestBoot}. -->
...
labels2plotBoot = labels2plot#c("Delta method SE (nonparametric bootstrap)", "Delta method SE (jackknife)", "Bootstrap SE (nonparametric)", "Percentile bootstrap", "BCa")
```

```{r sweepProcBiasSEboot, fig.cap = "Log10 of geometric mean of the ratios of estimated to the approximated true SE of the \\rsq over 1,000 Monte-Carlo instances (y-axis) for the .632 bootstrap as a function of estimation method (colour), sample size (x-axis), number of inner .632 bootstraps for MSE estimation (top panels), signal strength (side panels) and number of outer bootstraps (linetype). The results are similar to the CV results in the top panel of \\fref{fig:OneDim}.\\label{fig:biasUniBoot}", fig.widht = 7, fig.height = 7}
...
```{r risweepCovBoot, fig.cap = 'Coverage of the 95\\% confidence intervals for \\rsq across 1,000 Monte-Carlo instances (y-axis) for the .632 bootstrap for different sample sizes (x-axis), method (colour), signal strength (side panel) and number of inner .632 bootstrap replicates (top panels) for 500 repeats of the outer bootstrap. The results are similar to the CV case shown in \\fref{fig:OneDim}.\\label{fig:sweepCovBoot}'}
...
```{r plotwidthsweepBoot, fig.cap = 'Average width of the 95\\% confidence intervals for \\rsq across 1,000 Monte-Carlo instances (y-axis) for the .632 bootstrap for different sample sizes (x-axis), method (colour), signal strength (side panel) and number of .632 bootstrap replicates (top panels) for 500 outer bootstrap repeats.\\label{fig:sweepWidthBoot}'}
...
```{r CVEstimationHDboot, fig.height = 4, fig.cap = "Bias of the MSE estimation over 100 Monte-Carlo instances (y-axis) through cross-validation and .632 bootstrap (top panels) for the high-dimensional scenario as a function of sample size (x-axis) and signal strength (side panels) for 100 cross-validation splits and 200 bootstraps. The horizontal dotted line at 0 indicates no bias. For strong signals, the estimation of the MSE becomes difficult, since the true \\rsq may in some range increase quickly with sample size for high-dimensional models, and resampling algorithms fail to detect this.\\label{fig:biasMSEHD}", fig.width = 4.2}
...
#### The .632 bootstrap
<!-- .632 bootstrap results for the high-dimensional scenario are shown in Figures \ref{fig:biasHighBoot}-\ref{fig:zTestHDBoot}. -->
...
```{r plotwidthsweepHDboot, fig.cap = 'Average width of the 95\\% confidence intervals for the .632 bootstrap \\rsq (y-axis) for different sample sizes (x-axis), method (colour), signal strength (side panel) and number of bootstrap replicates (top panels) for 30 outer bootstrap samples for the high-dimensional scenario. Some graphs are truncated when all confidence interval estimations failed for a certain setting.\\label{fig:sweepWidthHDBoot}'}
...
```{r covMSEMST, eval = FALSE, fig.cap = '\\label{fig:covMSEMST}Scatterplot of MST estimates vs MSE estimates on 5,000 independent test datasets under the null scenario. The title shows the correlation.', fig.width = 4}
mse1 = vapply(FUN.VALUE = double(1), trueMSEnullSingle[["p2"]], function(w){w$MSE[1]})
plot(mse1, margVars, xlab = "Estimated MSE", ylab = "Estimated MST", 
     main = paste('Correlation:', round(digits = 3, cor(mse1, margVars))))
```
